...
class ServerDao extends DatabaseAccessor<AppDatabase> with _$ServerDaoMixin {
  ...
  Future<void> setActiveServer(String serverId) async {
    await transaction(() async {
      ...
      await (update(serversTable)..where((t) => t.id.equals(serverId))).write(
        const ServersTableCompanion(isActive: Value(true)),
      );
    });
  }
  ...
  Future<int> deleteServer(String serverId) {
    return (delete(serversTable)..where((t) => t.id.equals(serverId))).go();
  }
